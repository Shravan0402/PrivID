...
import { deployPrivIdFixture } from "./confidentialerc20.fixture";

describe("PrivId tests", function () {
  ...
  beforeEach(async function () {
    const contract = await deployPrivIdFixture();
    this.contractAddress = await contract.getAddress();
    this.privId = contract;
    this.instances = await createInstances(this.signers);
  });

  it("should be able to generate private ids", async function () {
    const transaction = await this.privId.mint(1000);
